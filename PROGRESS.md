...
### Фронтенд / UI
- [x] Фильтр по предприятию на странице Поставок (выпадающий список с названиями)
- [x] **Статистика сортности на странице Поставок** — справа от кнопки «Фильтры» показывать % каждого сорта по текущей выборке (с учётом всех активных фильтров и выбранного предприятия). Данные брать из `calculated_grade` по отфильтрованным поставкам.
- [ ] **Добавление предприятий** — на странице Предприятия добавить кнопку «+ Добавить предприятие» с формой ввода реквизитов (название, ИНН, регион, директор, телефон и т.д.). Новое предприятие сохраняется в БД.
...
### Backend / БД
- [ ] **Справочники нормативных показателей** — добавить в БД таблицу с описанием каждого показателя качества молока: название, единица измерения, методы измерения (ПО / Т001-2), нормативный документ, допустимые диапазоны, пояснение. Отображать на вкладке Нормативы рядом с таблицей граничных значений.
- [ ] **Загрузка данных через Excel** — добавить на странице Поставки (или отдельную страницу) возможность загружать xlsx-файл стандартной формы Quality Monitor. Backend принимает файл, парсит через `import_xlsx.py` и добавляет партии в БД. Показывать прогресс и результат импорта (сколько добавлено / пропущено / с ошибками).
- [x] Добавить скрипт применения всех миграций одной командой (`migrate_all.sql`)
