...
### 1. Раздел «Аналитика»
Привести в соответствие с вкладками файла Quality Monitor:

- **Сводная аналитика** — динамика ключевых показателей (SCC, КМАФАнМ, жир, белок) по месяцам и годам; сравнение предприятий; распределение по сортам в виде диаграмм
- **Поголовье** — динамика поголовья и удоя по предприятиям (если данные есть в БД)
- **Динамика по показателям** — выбор любого показателя → линейный график по месяцам за выбранный период, с линией норматива
- **Сравнение предприятий** — несколько предприятий на одном графике по выбранному показателю
- **Топ/Антитоп** — лучшие и худшие партии/периоды по выбранному показателю

Технически: новые эндпоинты аналитики с группировкой по месяцам, компоненты Recharts.

---

### 2. Отчёт в PDF
Скачивание стандартного отчёта (аналог вкладки Report в Quality Monitor) за указанный период:

- Пользователь выбирает: предприятие + период (от/до или год/квартал)
- Backend формирует PDF с таблицей показателей по периодам и нормативами
- Возможные форматы: PDF (основной), Excel (опционально)

Технически: backend генерирует PDF через `reportlab` или `weasyprint`; кнопка «Скачать отчёт» на карточке предприятия и на странице Аналитика.
